...
{				
	...
	char stack[PAGE_SIZE];		// 从堆栈段寄存器ss 可以获得其数据段选择符。PAGE_SIZE是4KB
};

static union task_union init_task = { INIT_TASK, };				// 定义初始任务的数据(sched.h 中)，进程0的task_struct
...
struct task_struct *task[NR_TASKS] = { &(init_task.task), };	// 定义任务指针数组。初始化进程槽task[NR_TASKS]的第一项为
																// 进程0，即task[0]为进程0占用
...
void sched_init (void)
{
	...
	p = gdt + 2 + FIRST_TSS_ENTRY;			// 从GDT的6项即TSS1开始，向上全部清0，
	for (i = 1; i < NR_TASKS; i++) 			// 并将进程槽从1往后的项全部清空
	{
		task[i] = NULL;
		p->a = p->b = 0;
		p++;
	...
	lldt (0);			// 将局部描述符表(LDT)加载到局部描述符表寄存器(LDTR)。
	...
	set_intr_gate (0x20, &timer_interrupt);
 	// 修改中断控制器屏蔽码，允许时钟中断。进程调度的基础
	outb (inb_p (0x21) & ~0x01, 0x21);
  	// 设置系统调用中断门。即设置系统调用总入口
