...
static _inline _syscall0(int, pause)				// int pause()系统调用：暂停进程的执行，直到收到一个信号。
static _inline _syscall1(int, setup, void *, BIOS)	// int setup(void * BIOS)系统调用，仅用于linux 初始化（仅在这个程序中被调用）。
...
#include <stdarg.h>    				// 标准参数头文件。以宏的形式定义变量参数列表。主要说
									// 明了-个类型(va_list)和三个宏(va_start, va_arg 和va_end)，vsprintf、vprintf、vfprintf。
...
{    
	...
	// 以下是内核进行所有方面的初始化工作
    mem_init(main_memory_start, memory_end); // 内存初始化（mm/memery.c）
	...
	// 回就绪运行态，但任务0（task0）是唯一的意外情况（参见'schedule()'），
	// 因为任务0 在任何空闲时间里都会被激活（当没有其它任务在运行时），
	// 因此对于任务0'pause()'仅意味着我们返回来查看是否有其它任务可以运行，
