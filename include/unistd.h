...
#define _POSIX_CHOWN_RESTRICTED		// chown()和fchown()的使用受限于进程的权限。只有超级用户可以执行chown（我想..）
#define _POSIX_NO_TRUNC				// 长于(NAME_MAX)的路径名将产生错误，而不会自动截断。路径名不截断（但是请看内核代码）
#define _POSIX_VDISABLE '\0'		// 下面这个符号将定义成字符值，该值将禁止终端对其的处理。禁止象^C 这样的字符
#define _POSIX_SAVED_IDS 			// 每个进程都有一保存的set-user-ID 和一保存的set-group-ID。我们将着手对此进行处理
#define _POSIX_JOB_CONTROL			// 系统实现支持作业控制。我们还没有支持这项标准，希望很快就行
...
// 更多的（可能的）可配置参数 - 现在用于路径名 下面的符号常数用于pathconf()函数
