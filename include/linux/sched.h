...
#define FIRST_TSS_ENTRY 4											// 全局表中第1 个任务状态段(TSS)描述符的选择符索引号。
#define FIRST_LDT_ENTRY (FIRST_TSS_ENTRY+1)							// 全局表中第1 个局部描述符表(LDT)描述符的选择符索引号。
#define _TSS(n) ((((unsigned long) n)<<4)+(FIRST_TSS_ENTRY<<3))		// 宏定义，计算在全局表中第n 个任务的TSS 描述符的索引号（选择符）。
#define _LDT(n) ((((unsigned long) n)<<4)+(FIRST_LDT_ENTRY<<3))		// 宏定义，计算在全局表中第n 个任务的LDT 描述符的索引号。
//#define ltr(n) __asm__( "ltr %%ax":: "a" (_TSS(n)))				// 宏定义，加载第n 个任务的任务寄存器tr。
...
// #define lldt(n) __asm__( "lldt %%ax":: "a" (_LDT(n)))				// 宏定义，加载第n 个任务的局部描述符表寄存器ldtr。
